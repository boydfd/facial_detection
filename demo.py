...
def format_image(image):
    ...
    image = image[face_coor[1]:(face_coor[1] + face_coor[2]),
...
def face_dect(image):
    ...
    for face in faces:
        if face[2] * face[3] > max_face[2] * max_face[3]:
            max_face = face
    face_image = image[max_face[1]:(max_face[1] + max_face[2]),
                 max_face[0]:(max_face[0] + max_face[3])]
    try:
        ...
        if result is not None:
            ...
            for c in range(0, 3):
                frame[200:320, 10:130, c] = emoji_face[:, :, c] * (
                        emoji_face[:, :, 3] / 255.0) + frame[200:320, 10:130,
                                                       c] * (
                                                    1.0 - emoji_face[:, :,
                                                          3] / 255.0)
